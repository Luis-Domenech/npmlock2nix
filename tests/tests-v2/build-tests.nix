...
in
symlinkAttrs {
  webpack-cli-project-default-build-command = npmlock2nix.v2.build {
  ...
  webpack-cli-project-custom-build-command = npmlock2nix.v2.build {
    src = ./examples-projects/webpack-cli-project;
  ...
  node-modules-attributes-are-passed-through = npmlock2nix.v2.build {
  ...
  passsing-buildInputs-doesnt-break-the-build = npmlock2nix.v2.build {
    src = ./examples-projects/webpack-cli-project;
    installPhase = ''
      cp -r dist $out
    '';
