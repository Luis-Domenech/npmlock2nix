...
testLib.makeIntegrationTests {
  leftpad = {
    ...
    shell = npmlock2nix.v1.shell { src = ../examples-projects/single-dependency; };
    command = ''
  ...
  pathContainsNodeApplications = {
    ...
    shell = npmlock2nix.v1.shell { src = ../examples-projects/bin-project; };
    command = ''
      mkdirp --version
    '';
    expected = "1.0.4\n";
