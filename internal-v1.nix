{
  ...
  default-nodejs ? null,
  alternate-nodejs ? null,
}:
let

  # Based on: https://stackoverflow.com/a/68934860 and https://lazamar.co.uk/nix-versions/?channel=nixpkgs-unstable&package=nodejs
  # This website also helps: https://www.nixhub.io/packages/nodejs
  # This version has pkgs.nodejs as version 12.18.3 and pkgs.nodejs_latest as 14.8.0
  nodejs-versions =
    let
      old-pkgs =
        import
          (builtins.fetchGit {
            name = "nodejs_default-12.18.3_latest-14.8.0";
      ...
      default-version = if default-nodejs ? version then default-nodejs.version else pkgs.nodejs.version;
    in
    # Check the version of the provided or nodejs or the default nodejs package from nixpkgs if none was provided
    if pkgs.lib.versionAtLeast default-version "15.0.0" then
      # The default/provided nodejs is at least or above version 15, so we fallback to the nodejs versions from the old nixpkgs
      {
        default = old-pkgs.nodejs;
        alternate = old-pkgs.nodejs_latest;
      }
    else
      {
        default = if default-nodejs != null then default-nodejs else pkgs.nodejs;
        alternate = if alternate-nodejs != null then alternate-nodejs else pkgs.nodejs_latest;
      };
...
rec {
  ...
  default-nodejs = nodejs-versions.default;

  # Used for testing
  alternate-nodejs = nodejs-versions.alternate;
  ...
  buildTgzFromGitHub =
    ...
    in
    if sourceOptions ? sourceOverrides.${name} then
      sourceOptions.sourceOverrides.${name} sourceInfo drv
    else
      drv;
  ...
  makeGithubSource =
    sourceOptions@{ sourceHashFunc, ... }:
    name: dependency:
    assert !(dependency ? version) -> builtins.throw "version` attribute missing from `${name}`";
    assert
      (lib.hasPrefix "github: " dependency.version)
      -> builtins.throw "invalid prefix for `version` field of `${name}` expected `github:`, got: `${dependency.version}`.";
    let
    ...
    assert
      v.org != f.org
      -> throw "version and from of `${name}` disagree on the GitHub org to fetch from: `${v.org}` vs `${f.org}`";
    assert
      v.repo != f.repo
      -> throw "version and from of `${name}` disagree on the GitHub repo to fetch from: `${v.repo}` vs `${f.repo}`";
  ...
  # Type: Set -> Bool
  shouldUseVersionAsUrl =
    dependency:
    dependency ? version
    && dependency ? integrity
    && !(dependency ? resolved)
    && looksLikeUrl dependency.version;
  ...
  makeUrlSource =
    ...
    // lib.optionalAttrs (sourceOverrides ? ${name}) {
  ...
  makeSource =
    sourceOptions: name: dependency:
    assert
      (builtins.typeOf name != "string") -> throw "Name of dependency ${toString name} must be a string";
    assert
      (builtins.typeOf dependency != "set")
    ...
    if dependency ? resolved && dependency ? integrity then
      ...
      throw "A valid dependency consists of at least the resolved and integrity field. Missing one or both of them for `${name}`. The object I got looks like this: ${builtins.toJSON dependency}";
  ...
  patchRequires =
    sourceOptions: name: requires:
    let
      patchReq =
        name: version:
        if lib.hasPrefix "github:" version then stringToTgzPath sourceOptions name version else version;
    in
  ...
  patchDependency =
    path: sourceOptions: name: spec:
    assert
      (builtins.typeOf name != "string") -> throw "Name of dependency ${toString name} must be a string";
    assert (builtins.typeOf spec != "set") -> throw "spec of dependency ${toString name} must be a set";
    let
      isBundled = spec ? bundled && spec.bundled == true;
      hasGitHubRequires =
        spec: (spec ? requires) && (lib.any (x: lib.hasPrefix "github:" x) (lib.attrValues spec.requires));
      patchSource = lib.optionalAttrs (!isBundled) (makeSource sourceOptions name spec);
      patchRequiresSources = lib.optionalAttrs (hasGitHubRequires spec) {
        requires = (patchRequires sourceOptions name spec.requires);
      };
      nestedDependencies = lib.mapAttrs (
        name: patchDependency (path ++ [ name ]) sourceOptions name
      ) spec.dependencies;
      patchDependenciesSources = lib.optionalAttrs (spec ? dependencies) {
        dependencies = lib.mapAttrs (_: value: value.result) nestedDependencies;
      };
      nestedIntegrityUpdates = lib.concatMap (value: value.integrityUpdates) (
        lib.attrValues nestedDependencies
      );
    ...
    in
    {
      result = result;
      integrityUpdates =
        lib.optional (result ? resolved && result ? integrity && result.integrity == null)
          {
            inherit path;
  ...
  patchPackagefile =
    ...
    let
      ...
      patchDep = (
        ...
        if lib.hasPrefix "github:" version then "*" else version
      );
      dependencies = if (content ? dependencies) then lib.mapAttrs patchDep content.dependencies else { };
      devDependencies =
        if (content ? devDependencies) then lib.mapAttrs patchDep content.devDependencies else { };
    in
  ...
  patchedPackagefile = file: writeText "package.json" (builtins.toJSON (patchPackagefile file));
  ...
  nodeSource =
    nodejs:
    runCommand "node-sources-${nodejs.version}" { } ''
  ...
  add_node_modules_to_cwd =
    ...
    + (
      if mode == "copy" then
        ...
        throw "node_modules_mode must be either `copy` or `symlink`"
    )
    + ''
  ...
  sourceHashFunc =
    githubSourceHashMap: spec:
    if spec.type == "github" then
      lib.attrByPath [ spec.value.org spec.value.repo spec.value.rev ]
        (lib.traceSeq "[npmlock2nix] warning: missing attr in githubSourceHashMap: ${spec.value.org}.${spec.value.repo}.${spec.value.rev}" null)
        githubSourceHashMap
    else
  ...
  node_modules =
    ...
    assert
      lib.versionAtLeast nodejs.version "15.0"
    ...
    assert
      (builtins.typeOf preInstallLinks != "set")
